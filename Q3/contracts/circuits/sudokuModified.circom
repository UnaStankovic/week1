...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rpPuz[i][j] = RangeProof(32);
            rpPuz[i][j].in <== puzzle[i][j];
            rpPuz[i][j].range[0] <== 0;
            rpPuz[i][j].range[1] <== 9;

            assert(rpPuz[i][j].out == 1); //[assignment] change assert() to use your created RangeProof instead
            rpSol[i][j] = RangeProof(32);
            rpSol[i][j].in <== solution[i][j];
            rpSol[i][j].range[0] <== 0;
            rpSol[i][j].range[1] <== 9;

            assert(rpSol[i][j].out == 1); //[assignment] change assert() to use your created RangeProof instead
